...
function App({ user, token, onLogout, initialFormId, onInitialFormLoaded }) {
                ...
                if (hasRealisasiFlag || hasInlineRealisasi) {
                    ...
                    try {
                        ...
                        if (realCheck.ok) {
                            ...
                            setRealizationFormId(realizations.length > 0 ? realizations[0].id : null);
                        }
    ...
    const handleExportExcel = () => {
        ...
        // Track which rows have item data for SUM formula
        ...
        items.forEach(main => {
            ...
            main.subs.forEach(sub => {
                ...
                const rowData = [`  ${sub.name}`, { t: 'n', v: sub.qty }, { t: 'n', v: sub.mdy },
                    { t: 'n', v: sub.qty * sub.mdy * sub.internalRate, z: acctFormat },
                    { t: 'n', v: sub.qty * sub.mdy * sub.rate, z: acctFormat }];
                if (activeTab === 'realisasi') rowData.push({ t: 'n', v: sub.actualRate || 0, z: acctFormat });
                wsData.push(rowData);
            });
        });

        wsData.push([]); // empty row before subtotal

        // SUM formula for subtotal using actual row indices
        const sumRange = (col) => {
            if (itemRowIndices.length === 0) return '0';
            const minR = Math.min(...itemRowIndices);
            const maxR = Math.max(...itemRowIndices);
            const startCell = XLSX.utils.encode_cell({ r: minR, c: col });
            const endCell = XLSX.utils.encode_cell({ r: maxR, c: col });
        ...
        if (activeTab === 'realisasi') {
            ...
            wsData.push(['', '', '', 'Actual Budget (Realisasi)', { t: 'n', f: cellRef(grandTotalRowIdx, 5), v: subtotalRealisasi, z: acctFormat }]);
            wsData.push(['', '', '', 'P/L (Realisasi)', { t: 'n', f: `${cellRef(grandTotalRowIdx, 5)}-${cellRef(grandTotalRowIdx, 3)}`, v: _plRealisasi, z: acctFormat }]);
        }
                ...
                <table className="data-table">
                    <thead>
                        <tr>
                            ...
                            {activeTab === 'realisasi' && <th className="col-realisasi" style={{ width: '20%' }}>Realisasi</th>}
                            {(activeTab === 'po' || activeTab === 'realisasi') && <th className="col-po" style={{ width: activeTab === 'po' ? '120px' : '100px' }}>PO Number</th>}
                ...
                <div className="metrics-section" style={{ marginTop: 0, flexBasis: '400px' }}>
                    ...
                    {activeTab === 'realisasi' && (
                        <>
                            ...
                            <div className="metric-line"><span className="metric-label" style={{ color: 'var(--primary)', fontWeight: 'bold' }}>Internal Budget</span><span className="metric-value" style={{ color: 'var(--primary)', fontWeight: 'bold' }}>{formatCurrency(grandTotalInternal)}</span></div>
